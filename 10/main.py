...
unique_trailheads = set()
for p in possible_paths:
    print(p)
    unique_trailheads.add(p[-1])

total_unique_paths = 0
for t in unique_trailheads:
    all_paths_for_trailhead = list(filter(lambda p : p[-1] == t, possible_paths))
    unique_9_heights = set()
    for p in all_paths_for_trailhead:
        unique_9_heights.add(p[0])
    total_unique_paths += len(unique_9_heights)

print("total_unique_paths", total_unique_paths)
